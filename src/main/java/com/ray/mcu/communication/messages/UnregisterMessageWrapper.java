...
public class UnregisterMessageWrapper extends AbstractMessageWrapper
{
    ...
    {
        super(sender.getServerData().getId(), MessageProto.Message.newBuilder().setUnregMsg(message).setSig(ByteString.copyFrom(sender.signMessage(message.toByteArray()))).build());
    }
     ...
     * @param sender the sender.
     */
    public UnregisterMessageWrapper(final int sender, final MessageProto.Message message)
    {
